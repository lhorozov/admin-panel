const { ProductStatus } = require("../lib/Enums");
...
function transformSortInput({ feature, type }) {
  ...
  // sorting by featured
  if (feature === "FEATURED") return { isFeatured: -1, createdAt: -1 };
...
function applyFilter4Theme(query, { brands, productCategories, hashtags }) {
  if (!query.$and) {
    query.$and = [{ isDeleted: false, wholesaleEnabled: { $ne: true } }];
  }
  const $or = [];
  if (brands.length) {
...
class ProductRepository {
  ...
  async isShippingBoxInUse(boxId) {
    return this.model
      .findOne({ shippingBox: boxId })
      .then((product) => product !== null);
  }

  async create(data) {
    if (data.description && data.description.length > 1023) {
  ...
  async createFromCSV(data) {
    ...
    if (data.customCarrier === undefined) {
      throw {
        errors: {
          customCarrier: {
            message: "customCarrier with that id doesn't exist",
          },
        },
      };
    }
  ...
  async getByHref(href) {
    ...
    return this.model.findOne({ href: href, isDeleted: false });
  }
}
