...
class AssetRepository {
  ...
  async getByName(filename) {
    return this.model.findOne({ filename });
  }
  ...
  async getByPathName(path) {
    return await this.model.findOne({ url: { $regex: ".*" + path + ".*" } });
  }
  ...
  async createFromUri(data) {
    return axios
      ...
      .then((response) => {
        const id = uuid();
        const { ext, type } = MIMEAssetTypes.detect(
  ...
  async createFromCSVForProducts(data) {
    const { url, path } = await buckets(data)
      .then((x) => x)
      .catch((err) => err);
