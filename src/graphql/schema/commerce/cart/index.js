...
const schema = gql`
  ...
  extend type Mutation {
    ...
    selectCartItems(ids: [ID]!, selected: Boolean = true): Cart!
      @auth(requires: USER)
    updateCartItemDeliveryRate(id: ID!, deliveryRate: ID!): CartProductItem!
      @auth(requires: USER)
    updateCartItemBillingAddress(
      ids: [ID!]
      billingAddress: ID!
    ): [CartProductItem!] @auth(requires: USER)
  }
`;
...
module.exports.resolvers = {
  ...
  Mutation: {
    ...
    clearCart: async (...args) => {
      ...
      return repository.userCartItem
        .clear(user.id, selected)
        .then(() => loadCart(...args));
    },
    selectCartItems,
  ...
  Cart: {
    price: async ({ items }, args) =>
      Promise.all(
        items.map(
          async ({ quantity, product, metricUnit, productAttribute }) => {
            if (!product && !productAttribute) {
              return 0;
            }

            const amountOfMoney = CurrencyFactory.getAmountOfMoney({
              centsAmount: productAttribute
                ? productAttribute.price * quantity
            ...
            if (productAttribute) {
              if (
                args.currency &&
                args.currency !== productAttribute.currency
              ) {
                const amountOfMoney = CurrencyFactory.getAmountOfMoney({
                  centsAmount: productAttribute.price * quantity,
                  currency: productAttribute.currency,
                });
                return CurrencyService.exchange(
                  amountOfMoney,
                  args.currency
            ...
            if (product) {
              ...
              if (metricUnit) {
                let [selectedItem] = product.metrics.filter(
                  (metricItem) => metricItem.metricUnit === metricUnit
                );
              ...
              if (args.currency && args.currency !== itemCurrency) {
                const amountOfMoney = CurrencyFactory.getAmountOfMoney({
                  centsAmount: unitPrice * quantity,
        ...
        const centsAmount = itemsSum.reduce(
    ...
    deliveryPrice: async ({ items }, args) =>
      Promise.all(
        items.map(async ({ quantity, deliveryRate }) => {
          if (deliveryRate) {
            ...
            return deliveryRate.amount * quantity;
          }
          return 0;
        })
      ).then((itemsSum) => {
    ...
    total: async ({ items }, args) =>
      Promise.all(
        items.map(async ({ quantity, product, deliveryRate, metricUnit }) => {
          let value = 0;
          if (product) {
            let unitPrice = product.price;
            let itemCurrency = product.currency;
            ...
            if (args.currency && args.currency !== itemCurrency) {
              ...
              value += await CurrencyService.exchange(
                amountOfMoney,
                args.currency
              ).then((exchangedMoney) => exchangedMoney.getCentsAmount());
            } else {
          ...
          if (deliveryRate) {
            if (args.currency && args.currency !== deliveryRate.currency) {
              const amountOfMoney = CurrencyFactory.getAmountOfMoney({
                centsAmount: deliveryRate.amount,
                currency: deliveryRate.currency,
              });
              value += await CurrencyService.exchange(
                amountOfMoney,
  ...
  CartProductItem: {
    total: async (
      ...
      if (metricUnit && product.metrics && product.metrics.length) {
        ...
        if (!selectedItem) {
          throw new UserInputError(
            `Product with id "${product._id}" does not have metric unit ${metricUnit}!`,
            { invalidArgs: [product] }
          );
        }
      ...
      if (currency !== itemCurrency) {
        const amountOfMoney = CurrencyFactory.getAmountOfMoney({
          centsAmount: productTotal,
          currency: itemCurrency,
        });
        productTotal = await CurrencyService.exchange(
          amountOfMoney,
          currency
        ).then((exchangedMoney) => exchangedMoney.getCentsAmount());
      }
      ...
      if (deliveryRate && currency !== deliveryRate.currency) {
        ...
        deliveryTotal = await CurrencyService.exchange(
          amountOfMoney,
          currency
        ).then((exchangedMoney) => exchangedMoney.getCentsAmount());
      }
      ...
      return CurrencyFactory.getAmountOfMoney({
        centsAmount: productTotal + deliveryTotal,
        currency,
      });
    },
    seller: async ({ product }, _, { dataSources: { repository } }) =>
      repository.product
        .getById(product)
        .then((product) => repository.user.getById(product.seller)),
    deliveryIncluded: ({ deliveryRate }) =>
      deliveryRate != null && typeof deliveryRate !== "undefined",
    deliveryAddress: async (
      ...
      return repository.deliveryRate
        .getById(rateId)
        .then((deliveryRate) =>
          repository.deliveryAddress.getById(deliveryRate.deliveryAddress)
        );
    },
    billingAddress: ({ billingAddress }, _, { dataSources: { repository } }) =>
      repository.billingAddress.getById(billingAddress),
    product: async (cartItem, _, { dataSources: { repository } }) =>
      repository.product.getById(cartItem.product),
    productAttribute: async (cartItem, _, { dataSources: { repository } }) =>
      repository.productAttributes.getById(cartItem.productAttribute),
  },
  CartItemInterface: {
