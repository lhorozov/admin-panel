...
module.exports = async (_, { id, productId }, { dataSources: { repository } }) => {
  ...
  validator.addPostRule(async (provider) => Promise.all([
    ...
    .then(([foundProductAttr, foundProduct]) => {
      let i = -1;
      if (!foundProduct) {
        provider.error('id', 'custom', `Product with id "${provider.inputs.productId}" does not exist!`);
      }
      if (!foundProductAttr) {
        provider.error('id', 'custom', `ProductAttr with id "${provider.inputs.id}" does not exist!`);
      }
      foundProduct.attrs.map((item, index) => {
        if (item === provider.inputs.id) i = index;
      });
      if (i === -1) {
        provider.error('id', 'custom', `ProductAttr with id "${provider.inputs.id}" does not exist in Product attributes!`);
      }
