...
const addProduct = async (product, index) => {
  ...
  product.isFeatured = product.isFeatured === 'TRUE' || product.isFeatured === 'true' ? true : false
  ...
  product.customCarrier = await new Promise((resolve) => repository.customCarrier.getById(product.customCarrier).then(async (res) => {
    ...
    console.log("customCarrier", res)
    resolve(product.customCarrier = res._id || res);
  }).catch(async () => {
    let org=await repository.organization.getByOwner(product.seller)
    console.log("org.customCarrier", org.customCarrier)
    resolve(org.customCarrier);
  }));
  ...
  try {
    product.attrs = [];
    let attributeNames1
    if (attributeNames.indexOf(';') > -1) {
      attributeNames1 = attributeNames.split(';');
    } else {
      attributeNames1 = [attributeNames]
    }

    let attributeValues1
    if (attributeValues.indexOf(';') > -1) {
      attributeValues1 = attributeValues.split(';');
    } else {
      attributeValues1 = [attributeValues]
    }
    for (let i = 0; i < attributeValues1.length; i++) {
      let oneValue = [attributeValues1[i]];
      if (attributeValues1[i].includes('|')) {
        oneValue = attributeValues1[i].split('|');
      }
      const attributeData = {};
      ...
      attributeData.asset = await new Promise((resolve) => repository.asset.createAssetFromCSVForProducts(assetData)
        .then((res) => {
          resolve(res._id);
        }).catch(() => {
          resolve(null);
    ...
    console.log('error occured while add productAttributes', error);
  }

  try {
    product.metaDescription = `Buy ${product.title} and other `;
    if (product.brand) {
      product.metaDescription += `${brand_name} `;
    }
    if (product.category) {
      const productCategory = await repository.productCategory.getById(product.category);
      if (productCategory && productCategory.parent) {
        const parentCategories = [];
        for (let i = 0; i < productCategory.parents.length; i++) {
          const category = productCategory.parents[i];
          if (category) {
            parentCategories.push(await repository.productCategory.getById(category));
          }
        }
        parentCategories.push(await repository.productCategory.getById(productCategory.parent));

        parentCategories.map((category) => {
          product.metaDescription += `${category.name}, `;
        });
        product.metaDescription += productCategory.name;
      }

      product.metaDescription += ` at ${shopName}, starting at `;

      const formattedPrice = CurrencyFactory.getAmountOfMoney({ currencyAmount: product.price / 100, currency: product.currency }).getFormatted();
      product.metaDescription += `${formattedPrice}!`;
    ...
    console.log('error occured while add meta description', error);
  }

  try {
    product.metaTitle = product.title;
    if (product.metaTitle && product.metaTitle.length > 49) {
      product.metaTitle = product.metaTitle.substring(0, 44).concat('...');
    }
    product.metaTitle += ` | ${shopName}`;
  } catch (error) {
    console.log('error occured while add meta description', error);
  }

  try {
    product.imageAltText = product.title;
    if (product.imageAltText && product.imageAltText.length > 49) {
      product.imageAltText = product.imageAltText.substring(0, 44).concat('...');
    }
    product.imageAltText += ` | ${shopName}`;
  } catch (error) {
    console.log('error occured while add meta description', error);
  }
  ...
  if(product.customCarrier===null){
    let org=await repository.organization.getByOwner(product.seller)
    console.log("org.customCarrier", org.customCarrier)
    product.customCarrier=org.customCarrier
  }
  
  console.log("adding product================>", product)
  return await repository.product.createFromCSV(product).then((res) => {
    console.log("added product from createFromCSV", res)
