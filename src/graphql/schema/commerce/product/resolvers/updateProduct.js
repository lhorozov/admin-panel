...
const { InventoryLogType, LanguageList, ProductStatus } = require(path.resolve('src/lib/Enums'));
...
const updateDraftProduct = require('./updateDraftProduct');
...
const activity = {
  translateProduct: async (product, repository) => {
    ...
    return Promise.all(languageList.map(async (language) => {
  ...
  getProductInventoryLog: async ({ id, quantity }, repository) => repository.productInventoryLog.getByProductId(id)
    .then((inventory) => {
      ...
      return repository.productInventoryLog.add(inventoryLog);
    }),
};

module.exports = async (_, { id, data }, { dataSources: { repository }, user }) => {
  if (data.status === ProductStatus.DRAFT) {
    return updateDraftProduct(null, { id, data }, { dataSources: { repository }, user });
  }
  ...
  return validator.check()
    ...
    .then(async () => {
      ...
      product.status = quantity === 0 ? ProductStatus.INACTIVE : data.status;
