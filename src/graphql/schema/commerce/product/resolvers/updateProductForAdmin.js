...
const activity = {
  translateProduct: async (product, repository) => {
    return Promise.all([
      ...
      return repository.productTranslation.updateProduct(product.id, {
        title,
        description,
      });
    });
...
module.exports = async (
    ...
    Promise.all([
      ...
      if (!foundProduct) {
        provider.error(
          "id",
          "custom",
          `Product with id "${provider.inputs.id}" doen not exist!`
        );
      }
      ...
      if (!category) {
        provider.error(
          "category",
          "custom",
          `Category with id "${provider.inputs.category}" doen not exist!`
        );
      }
      ...
      if (!brand) {
        provider.error(
          "brand",
          "custom",
          `Brand with id "${provider.inputs.brand}" doen not exist!`
        );
      }
  ...
  return validator
    ...
    .then(async () => {
      ...
      const { quantity, price, discountPrice, ...productData } = data;

      product.title = productData.title;
      product.description = productData.description;
      product.price = CurrencyFactory.getAmountOfMoney({
        currencyAmount: data.discountPrice || data.price,
        currency: data.currency,
      }).getCentsAmount();
      product.oldPrice = data.discountPrice
      ...
      product.customCarrierValue = customCarrier
        ? CurrencyFactory.getAmountOfMoney({
            currencyAmount: data.customCarrierValue,
            currency: data.currency,
          }).getCentsAmount()
        : 0;
      ...
      if (productData.thumbnailId) {
        ...
        if (thumbnail) {
          productData.thumbnail = productData.thumbnailId;
        } else {
          throw new ForbiddenError(
            `Thumbnail with id "${productData.thumbnailId}" does not exist!`
          );
        }
      }

      const amountOfMoney = CurrencyFactory.getAmountOfMoney({
        centsAmount: data.price,
        currency: data.currency,
      });
      ...
      return Promise.all([
        ...
        await repository.productInventoryLog.update(
          inventory.id,
