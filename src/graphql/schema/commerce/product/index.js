...
const { ProductMetricUnits, ProductStatus } = require(path.resolve(
  "src/lib/Enums"
));
...
const schema = gql`
  ...
  type Product {
    ...
    category: ProductCategory @ifDiffers(key: "status", value: "DRAFT")
    # weight: Weight!
    shippingBox: ShippingBox @ifDiffers(key: "status", value: "DRAFT")
    brand: Brand @ifDiffers(key: "status", value: "DRAFT")
  ...
  input ProductSortInput {
    feature: ProductSortFeature! = FEATURED
...
module.exports.resolvers = {
  Query: {
    products,
    product: async (_, { id }, { dataSources: { repository } }) =>
      repository.product.getById(id),
    previewBulkProducts,
    productAttributes: async (
      _,
      { productId },
      { dataSources: { repository } }
    ) => repository.productAttributes.getByProduct(productId),
    productsByTheme,
    productBySlug: async (_, { slug }, { dataSources: { repository } }) =>
      repository.product.getBySlug(slug),
  },
  Mutation: {
    addProduct,
    updateProduct,
  ...
  Product: {
    ...
    customCarrierValue: async ({ customCarrierValue, currency }, args) => {
      const amountOfMoney = CurrencyFactory.getAmountOfMoney({
        centsAmount: customCarrierValue,
        currency,
      });
    ...
    relatedLiveStreams: async (
      { id },
      { limit },
      { dataSources: { repository } }
    ) =>
      repository.liveStream.get({
        filter: {
          experiences: [],
          categories: [],
          cities: [],
    ...
    title: async (
      ...
      return repository.productTranslation
        .getByProduct(id)
        .then((translation) =>
          translation && translation.title[language.toLowerCase()]
            ? translation.title[language.toLowerCase()]
            : title
        );
    },
    description: async (
      ...
      return repository.productTranslation
        .getByProduct(id)
        .then((translation) =>
          translation && translation.description[language.toLowerCase()]
            ? translation.description[language.toLowerCase()]
            : description
        );
    },
  ...
  ProductAttribute: {
    ...
    quantity: async ({ quantity }) =>
      typeof quantity === "number" ? Math.floor(quantity) : 0,
  },
  ProductMetricItem: {
    unitPrice: async ({ unitPrice }, args, { dataSources: { repository } }) => {
      let amountOfMoney = CurrencyFactory.getAmountOfMoney({
        centsAmount: unitPrice.amount,
