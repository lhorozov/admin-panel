...
module.exports.resolvers = {
  Query: {
    searchBrand,
    allBrands,
    brand: async (_, { id }, { dataSources: { repository } }) =>
      repository.brand.getById(id),
    brandBySlug: async (_, { slug }, { dataSources: { repository } }) =>
      repository.brand.getBySlug(slug),
  },
  Brand: {
    name: (
      { translations, name },
    ...
    banners: async ({ banners = [] }, _, { dataSources: { repository } }) =>
      repository.banner.getByIds(banners),
    featureProducts: async (
      { id, featureProducts },
      _,
      { dataSources: { repository } }
    ) =>
      repository.product.getByIds(featureProducts).then((products) => {
    ...
    featureCategories: async (
      { id, featureCategories },
      _,
      { dataSources: { repository } }
    ) =>
      repository.productCategory
        .findByIds(featureCategories)
        .then((categories) => {
