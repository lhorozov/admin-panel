...
module.exports = async (obj, args, { dataSources: { repository }, user }) => {
  ...
  return validator
    ...
    .then(() => {
      const experience = repository.liveStreamExperience.getById(
        args.data.experience
      );
      if (!experience) {
        throw new UserInputError(
          `Live Stream Experience ${args.data.experience} does not exist`,
    ...
    .then(() =>
      Promise.all(
        args.data.preview.map((assetId) => repository.asset.load(assetId))
      ).then((previews) => {
        if (previews && previews.length > 0) {
          previews
            ...
            .forEach((preview, i) => {
              throw new Error(
                `Preview can not be addded to the Live Stream, because of Asset "${args.data.preview[i]}" does not exist!`
              );
            });
        }
      })
    ...
    .then(() =>
      Promise.all([repository.asset.load(args.data.previewVideo)]).then(
        ([previewVideo]) => {
          if (args.data.previewVideo && !previewVideo) {
            throw new UserInputError(
              `Asset ${args.data.previewVideo} does not exist`,
              { invalidArgs: "preview" }
            );
          }
        }
    ...
    .then(() =>
      Promise.all(
        args.data.productDurations.map((productDuration) =>
          repository.product.getById(productDuration.product)
        )
      ).then((products) => {
        products.forEach((product, i) => {
          if (!product) {
            throw new Error(
              `Product can not be addded to the Live Stream, because of Product "${args.data.productDurations[i].product}" does not exist!`
            );
          }
        });
      })
    ...
    .then(() =>
      repository.asset.getById(args.data.thumbnail).then((thumbnail) => {
        if (!thumbnail) {
          throw new Error(
            `Thumbnail asset does not exist with id "${args.data.thumbnail}"!`
          );
        }
      })
    )
    ...
    .then(async () => {
      ...
      const channel = {
        ...
        status:
          args.data.liveStreamRecord.length > 0
    ...
    .catch((error) => {
      throw new ApolloError(
        `Failed to add Live Stream. Original error: ${error.message}`,
        400
      );
    });
};
