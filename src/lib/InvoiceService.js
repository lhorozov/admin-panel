...
module.exports.InvoiceService = {
  async getOrderDetails(orderID) {
    ...
    await Promise.all(itemsDetail.purchaseOrder.items.map(async (item) => {
      ...
      if (item.productAttribute && item.productAttribute.asset) {
        image = item.productAttribute.asset.url;
      } else if (item.product && item.product.assets && item.product.assets.length > 0) {
        image = item.product.assets[0].url;
      }
      ...
      image = product.product.assets.length > 0 ? product.product.assets[0].url : '';
